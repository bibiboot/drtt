                    ...
                    void* payload)
{
    ...
    reverse_kernel_timestamp(payload, CH_LEN + 2*TIMESTAMP_LEN);

    unsigned long local_xmit_time  = (unsigned long)EXTRACT_64BITS(local_tx);
    unsigned long remote_xmit_time = (unsigned long)EXTRACT_64BITS(remote_tx);
    unsigned long local_recv_time  = (unsigned long)local_rx->sec * SECONDS + local_rx->fsec * NANOSECONDS;
